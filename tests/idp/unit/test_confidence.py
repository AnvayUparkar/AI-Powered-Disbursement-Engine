...
def test_docling_parser_does_not_synthesise_confidence_from_text(monkeypatch):
    ...
    # Text shape no longer influences the score at all: compute_text_confidence is gone,
    # and with no layout prediction available both items report the same neutral value.
    assert result.elements[0].confidence == result.elements[1].confidence
    # The real model fields stay explicitly unknown rather than guessed.
...
def test_clean_bilingual_preserves_alphanumeric_field_values():
    ...
    # IFSC codes match the [A-Z]{2,}\d+ misread shape exactly and must survive both the
    # sanitizer and the garble check, or every bank statement silently loses them.
    assert clean("HDFC0000123") == "HDFC0000123"
    assert clean("IFSC Code SBIN0001234") == "IFSC Code SBIN0001234"
    assert OCRConfidenceEvaluator().is_garbled_text("HDFC0000123") is False
