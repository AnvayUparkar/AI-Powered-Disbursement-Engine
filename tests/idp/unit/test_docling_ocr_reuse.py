...
async def test_reuse_docling_ocr_skips_step4_rapidocr(monkeypatch):
    ...
    monkeypatch.setattr(processor, "_get_docling_parser", lambda doc_type, is_scanned=None: mock_docling_parser)
    ...
    assert len(ocr_results) == 0  # No standalone OCR results produced
