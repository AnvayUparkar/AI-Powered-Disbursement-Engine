...
class S3Storage:
    ...
    async def download(
        ...
        if client == "MOCK" or not os.getenv("AWS_ACCESS_KEY_ID"):
            ...
            if os.path.exists(mock_path):
                ...
                raise S3Error(f"Object not found in mock store: s3://{target_bucket}/{key}", details=mock_path)
    ...
    async def delete(self, key: str, bucket: Optional[str] = None) -> None:
        ...
        client = self._get_client()
        if client == "MOCK" or not os.getenv("AWS_ACCESS_KEY_ID"):
            mock_path = os.path.join(settings.TEMP_DIR, "s3_mock", target_bucket, key)
            if os.path.exists(mock_path):
                os.remove(mock_path)
                logger.info(f"[Mock] Deleted {mock_path}")
            return
        try:
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(None, lambda: client.delete_object(Bucket=target_bucket, Key=key))
        except Exception as e:
            raise S3Error(f"Failed to delete s3://{target_bucket}/{key}", details=str(e))

    async def delete_prefix(self, prefix: str, bucket: Optional[str] = None) -> None:
        """Delete all objects whose key starts with prefix."""
        target_bucket = bucket if (isinstance(bucket, str) and bucket.strip()) else self.default_bucket
        client = self._get_client()
        if client == "MOCK" or not os.getenv("AWS_ACCESS_KEY_ID"):
            import glob as _glob
            mock_base = os.path.join(settings.TEMP_DIR, "s3_mock", target_bucket)
            pattern = os.path.join(mock_base, prefix.replace("/", os.sep)) + "**"
            for match in _glob.glob(pattern, recursive=True):
                if os.path.isfile(match):
                    os.remove(match)
                    logger.info(f"[Mock] Deleted {match}")
            return
        # Real mode: paginate and batch delete
        paginator = client.get_paginator("list_objects_v2")
        for page in paginator.paginate(Bucket=target_bucket, Prefix=prefix):
            objects = [{"Key": o["Key"]} for o in page.get("Contents", [])]
            if objects:
                client.delete_objects(Bucket=target_bucket, Delete={"Objects": objects})
