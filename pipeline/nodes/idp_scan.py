...
def build_idp_result_from_parsed(parsed: ParsedDocument, doc_type: str, doc_id: str) -> Dict[str, Any]:
    ...
    for tbl in (parsed.tables or []):
        tables_data.append({
            ...
            "headers": tbl.headers,
            "rows": tbl.rows_raw,
            # Restored in the main merge: app/services/registry/case_scanner.py reads
            # bbox/cells/markdown off these records, so dropping them left the table
            # overlay and markdown view with nothing to render.
            "bbox": tbl.bbox,
