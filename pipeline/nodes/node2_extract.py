...
def get_processor() -> DocumentProcessor:
    """Returns the shared process-wide DocumentProcessor singleton."""
    return processor
...
def _process_file_with_idp(file_path: Path, doc_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if parsed:
            ...
            return build_idp_result_from_parsed(parsed=parsed, doc_type=doc_type, doc_id=doc_id)
...
def node2_extract(state: PipelineState) -> PipelineState:
    ...
    doc_stem_map: Dict[str, Dict[str, Any]] = {}
    ...
    if binary_doc_tasks:
        ...
        def _worker_task(task_tuple: Tuple[str, Path, str, str]) -> Tuple[str, Path, str, Optional[Dict[str, Any]]]:
            ...
            return fname, fpath, doc_key, res
        ...
        with ThreadPoolExecutor(max_workers=worker_count, thread_name_prefix="idp_doc_worker") as executor:
            ...
            for future in futures:
                fname, fpath, doc_key, idp_result = future.result()
                if idp_result:
                    extracted_data[doc_key] = idp_result
                    doc_stem_map[fpath.stem] = idp_result
                else:
        ...
        for json_file in extracted_dir.glob("*.json"):
            ...
            try:
                ...
                if key == "face_embeddings" and not face_embeddings:
                    ...
                    if key not in doc_stem_map and doc_key in extracted_data:
                        doc_stem_map[key] = extracted_data[doc_key]
    ...
    # Every document gets its distinct .json file formed and saved using the exact template format.
    ...
    docs_to_persist: Dict[str, Dict[str, Any]] = {}
    for doc_k, doc_v in extracted_data.items():
        if isinstance(doc_v, dict):
            docs_to_persist[doc_k] = doc_v
    for stem_k, doc_v in doc_stem_map.items():
        if isinstance(doc_v, dict) and stem_k not in docs_to_persist:
            docs_to_persist[stem_k] = doc_v

    for doc_k, doc_v in docs_to_persist.items():
        try:
            # Write exclusively the canonical 22-field template JSON for each document
            template_json = format_template_json(doc_v)
            write_json(extracted_out_dir / f"{doc_k}.json", template_json)
